...
const SHOP_ITEMS = {
  ...
  buff_luck: { name: 'Luck Potion', type: 'buff', price: 300, durationMs: 3600000, emoji: '🍀', description: 'Increases rare catch chance for 1 hour' }
};
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (item.type === 'buff') {
      ...
      const embed = new EmbedBuilder()
        .setTitle('✅ Purchase Successful')
        .setDescription(`You activated a **${item.name}**!`)
        .addFields(
          { name: 'Price Paid', value: formatPrice(item.price), inline: true },
          { name: 'New Balance', value: `$${userData.balance.toLocaleString()}`, inline: true },
          { name: 'Effect', value: 'Increased rare catch chance', inline: true },
